 ...
 * @author tommy
 */
public class DeletePenalty extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null && (user.getAccessLevel().equals("admin") ||
            ...
            try {
				...
				PreparedStatement pstmt = con.prepareStatement("delete from penalty " +
																"where penaltyid = ?"); 

                pstmt.setInt(1, Integer.valueOf(request.getParameter("penaltyid")));
				pstmt.execute();
				request.setAttribute("success", "The penalty has been deleted.");
			} catch (ClassNotFoundException e) {
				e.printStackTrace();
			} catch (SQLException e) {
				e.printStackTrace();
				request.setAttribute("error", "Error deleting the penalty.");
